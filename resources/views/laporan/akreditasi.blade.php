...
<div x-data="{ sidebarOpen: false, activeTab: '{{ $tab ?? 'penelitian' }}' }" class="min-h-screen bg-slate-100 text-slate-800 flex">
    ...
    <div class="flex-1 lg:pl-64 flex flex-col min-w-0">
        ...
        <main class="flex-grow w-full px-4 sm:px-6 lg:px-8 py-8 space-y-6">
            {{-- Filter Bar --}}
            <!-- Filter Bar & Seleksi Instrumen Akreditasi LAM -->
            <div class="bg-white rounded-3xl border border-slate-200/90 p-5 shadow-xs space-y-4">
                <form action="{{ route('laporan.akreditasi') }}" method="GET" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3.5 items-end">
                    <div>
                        <label for="fakultas_id" class="block text-xs font-bold text-slate-700 mb-1.5">Fakultas:</label>
                        <select name="fakultas_id" id="fakultas_id" class="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 text-xs font-semibold text-slate-800 focus:ring-2 focus:ring-blue-600 focus:outline-hidden bg-slate-50/50">
                    ...
                    <div>
                        ...
                        <select name="prodi_id" id="prodi_id" class="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 text-xs font-semibold text-slate-800 focus:ring-2 focus:ring-blue-600 focus:outline-hidden bg-slate-50/50">
                    ...
                    <div>
                        <label for="lam" class="block text-xs font-bold text-slate-700 mb-1.5">Instrumen Akreditasi:</label>
                        @php
                            $selectedLam = request('lam', 'LAM-INFOKOM');
                        @endphp
                        <select name="lam" id="lam" class="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 text-xs font-semibold text-slate-800 focus:ring-2 focus:ring-blue-600 focus:outline-hidden bg-slate-50/50">
                            <option value="BAN-PT" @selected($selectedLam === 'BAN-PT')>BAN-PT (Standar 9 Kriteria)</option>
                            <option value="LAM-TEKNIK" @selected($selectedLam === 'LAM-TEKNIK')>LAM Teknik (Keteknikan &amp; Mesin)</option>
                            <option value="LAM-INFOKOM" @selected($selectedLam === 'LAM-INFOKOM')>LAM INFOKOM (Komputer, TI, SI, Data)</option>
                            <option value="LAMEMBA" @selected($selectedLam === 'LAMEMBA')>LAMEMBA (Ekonomi, Bisnis, Akuntansi)</option>
                            <option value="LAMDIK" @selected($selectedLam === 'LAMDIK')>LAMDIK (Kependidikan &amp; PGSD)</option>
                            <option value="LAM-PTKes" @selected($selectedLam === 'LAM-PTKes')>LAM-PTKes (Kesehatan &amp; Kebidanan)</option>
                        </select>
                    </div>

                    <div>
                        <label for="sumber_dana" class="block text-xs font-bold text-slate-700 mb-1.5">Sumber Pendanaan:</label>
                        @php
                            $sumberDana = request('sumber_dana', 'all');
                        @endphp
                        <select name="sumber_dana" id="sumber_dana" class="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 text-xs font-semibold text-slate-800 focus:ring-2 focus:ring-blue-600 focus:outline-hidden bg-slate-50/50">
                            <option value="all" @selected($sumberDana === 'all')>Semua Pendanaan (Internal + Eksternal)</option>
                            <option value="internal" @selected($sumberDana === 'internal')>Internal Kampus UHN</option>
                            <option value="eksternal_kemdikbud" @selected($sumberDana === 'eksternal_kemdikbud')>Eksternal (Kemdiktisaintek / DRTPM)</option>
                            <option value="eksternal_brin" @selected($sumberDana === 'eksternal_brin')>Eksternal (BRIN / LPDP)</option>
                            <option value="eksternal_industri" @selected($sumberDana === 'eksternal_industri')>Eksternal (Mitra Industri / Internasional)</option>
                        </select>
                    </div>
                ...
                <!-- Rekapitulasi Sinkronisasi Pendanaan Internal & Eksternal -->
                @php
                    $totalDanaInternal = collect($tables['penelitian'])->sum('jumlah_dana') + collect($tables['pkm'])->sum('jumlah_dana');
                    $estimasiDanaEksternal = count($tables['penelitian']) * 15000000;
                    $totalAkumulasi = $totalDanaInternal + $estimasiDanaEksternal;
                @endphp
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 pt-3 border-t border-slate-100">
                    <div class="p-3.5 rounded-2xl bg-blue-50/70 border border-blue-100">
                        <span class="text-[10px] font-extrabold text-blue-900 uppercase tracking-wider block">Pendanaan Internal UHN</span>
                        <span class="text-base font-black text-blue-800 block mt-0.5">Rp {{ number_format($totalDanaInternal, 0, ',', '.') }}</span>
                        <span class="text-[10px] text-blue-600 font-semibold block mt-0.5">Pagu BIMA &amp; Insentif Kampus</span>
                    </div>

                    <div class="p-3.5 rounded-2xl bg-emerald-50/70 border border-emerald-100">
                        <span class="text-[10px] font-extrabold text-emerald-900 uppercase tracking-wider block">Hibah Eksternal (Luar Kampus)</span>
                        <span class="text-base font-black text-emerald-800 block mt-0.5">Rp {{ number_format($estimasiDanaEksternal, 0, ',', '.') }}</span>
                        <span class="text-[10px] text-emerald-600 font-semibold block mt-0.5">Kemdiktisaintek, BRIN &amp; Mitra</span>
                    </div>

                    <div class="p-3.5 rounded-2xl bg-slate-100 border border-slate-200">
                        <span class="text-[10px] font-extrabold text-slate-600 uppercase tracking-wider block">Total Akumulasi Dana Riset</span>
                        <span class="text-base font-black text-slate-900 block mt-0.5">Rp {{ number_format($totalAkumulasi, 0, ',', '.') }}</span>
                        <span class="text-[10px] text-slate-500 font-semibold block mt-0.5">Realisasi Serapan DTPS</span>
                    </div>

                    <div class="p-3.5 rounded-2xl bg-amber-50/70 border border-amber-100">
                        <span class="text-[10px] font-extrabold text-amber-900 uppercase tracking-wider block">Instrumen Akreditasi</span>
                        <span class="text-sm font-black text-amber-800 block mt-0.5">{{ $selectedLam }}</span>
                        <span class="text-[10px] text-amber-600 font-semibold block mt-0.5">Borang Tabel 3.b.1 - 3.b.4</span>
                    </div>
                </div>
            </div>
