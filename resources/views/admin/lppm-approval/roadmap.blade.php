        ...
        <main class="w-full px-4 sm:px-6 lg:px-8 py-8 space-y-5">
            ...
            @forelse($proposals as $proposal)
                <article class="bg-white rounded-3xl border border-slate-200 shadow-lg p-5 sm:p-6 space-y-5">
                    ...
                    <!-- Abstrak Usulan & Dokumen Proposal PDF -->
                    <div x-data="{ showAbstrak: false }" class="rounded-2xl bg-slate-50 border border-slate-200 p-4 text-xs space-y-2">
                        <div class="flex items-center justify-between">
                            <span class="font-extrabold text-slate-800 uppercase tracking-wider text-[11px]">Abstrak &amp; Ringkasan Penelitian:</span>
                            <button type="button" @click="showAbstrak = !showAbstrak" class="text-blue-700 font-bold hover:underline cursor-pointer">
                                <span x-text="showAbstrak ? 'Tutup Abstrak' : 'Buka &amp; Baca Abstrak Lengkap'"></span>
                            </button>
                        </div>
                        <div x-show="showAbstrak" class="text-slate-700 leading-relaxed font-normal whitespace-pre-line border-t border-slate-200/80 pt-2 text-xs">
                            {{ $proposal->ringkasan ?: 'Ringkasan belum diisi oleh pengusul.' }}
                        </div>
                        @if($proposal->file_proposal_path)
                            <div class="pt-1">
                                <a href="{{ route('usulan.document', $proposal) }}" target="_blank" rel="noopener" class="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white text-blue-700 border border-blue-200 text-xs font-bold hover:bg-blue-50 transition shadow-2xs">
