...
<div x-data="{ sidebarOpen: false }" class="min-h-screen bg-slate-100 text-slate-800 flex">
    ...
    <div class="flex-1 lg:pl-64 min-w-0">
        <header class="bg-white border-b border-slate-200 py-4 sticky top-0 z-30 shadow-sm">
            <div class="w-full px-4 sm:px-6 lg:px-8 flex items-center gap-3">
                    ...
                    <article class="bg-white rounded-3xl border border-slate-200 shadow-lg p-5 sm:p-6 space-y-5">
                        <div class="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_220px] gap-5">
                            <div class="min-w-0">
                                ...
                                {{-- Abstrak Mode View --}}
                                @if($proposal->ringkasan_substansi)
                                    <div x-data="{ showAbstrak: false }" class="mt-3">
                                        <button type="button" @click="showAbstrak = !showAbstrak" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-xs font-bold text-slate-700 transition">
                                            <svg class="w-4 h-4 text-blue-600 transition-transform" :class="{ 'rotate-90': showAbstrak }" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                                            <span x-text="showAbstrak ? 'Sembunyikan Abstrak Proposal' : 'Lihat Abstrak Proposal'"></span>
                                        </button>
                                        <div x-show="showAbstrak" x-cloak class="mt-2.5 p-4 rounded-2xl bg-slate-50 border border-slate-200 text-xs text-slate-700 leading-relaxed font-serif">
                                            <div class="font-sans font-bold text-slate-900 mb-1 flex items-center gap-1.5">
                                                <svg class="w-4 h-4 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                                                <span>Abstrak / Ringkasan Substansi:</span>
                                            </div>
                                            <div class="whitespace-pre-line">{{ $proposal->ringkasan_substansi }}</div>
                                        </div>
                                    </div>
                                @endif

                                @if($proposal->file_proposal_path)
                                    <a href="{{ route('usulan.document', $proposal) }}" target="_blank" rel="noopener" class="inline-flex items-center gap-2 mt-3 text-xs font-extrabold text-blue-700 hover:text-blue-900">
                                        ...
                                        <span>Buka dokumen proposal PDF</span>
                                    </a>
                                @endif
